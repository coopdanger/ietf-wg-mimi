...
Achieving interoperability requires a solution for the introduction problem, i.e., the ability for a user in one application to take an identifier for a target user, be granted permission to initiate communications, and be able to establish communications with the target. The working group will specify a solution to the introduction problem, together with best practice recommendations for functionality, configuration options, and other aspects.  The working group may investigate and develop a protocol to discover the target user's preferred application in which that target should be reached. Express user preferences about discoverability and reachability must be respected. 
