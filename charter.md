...
Achieving interoperability requires a solution for the introduction problem, i.e., given an identifier for a target user, a sending user should be able to establish communications with the target after the target user's service has been reached and the sender has been granted permission to initiate communications. The working group will specify a solution to the introduction problem, together with best practice recommendations for functionality, configuration options, and other aspects.

Some messaging services today have features which define how a user is contacted by someone new. Some services require the user's approval while others allow inbound messages from any other user. This group's work will respect the use of these features and allow them to work the same when inbound messages come from an interoperating service.
